...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 2.
        # Tutorial on packing and unpacking: 
        ...
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens)  # shape (src_len, b, h*2)
        enc_hiddens = enc_hiddens.permute(1,0,2)    # shape (src_len, b, h*2) -> (b, src_len, h*2)
        
        # 3.
        # concat first, then apply layers
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 3.
        for Y_t in torch.split(Y, 1, 0): # Y_t shape (1, b, e)
            Y_t = torch.squeeze(Y_t, dim=0) # Y_t shape (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # (b, src_len, h) .bmm(b, h, 1) -> (b, src_len, 1) -> (b, src_len)
